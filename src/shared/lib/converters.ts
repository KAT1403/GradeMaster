...
export const ECTS_VALUES: Record<string, number> = Object.fromEntries(
  KAZ_UNIVERSITY_SCALE.map((item) => [item.letter, item.gpa]),
);

export const getLetterFromGPA = (gpaVal: number): string => {
  const found = KAZ_UNIVERSITY_SCALE.find((item) => gpaVal >= item.gpa);
  return found?.letter ?? "F";
};
...
export const calculateIntlGPA = (percent: number): GPAInfo => {
  ...
  const found = KAZ_UNIVERSITY_SCALE.find(
    (item) => rounded >= item.min && rounded <= item.max,
  );
  return found ? { score: found.gpa, letter: found.letter } : { score: 0.0, letter: "F" };
};
