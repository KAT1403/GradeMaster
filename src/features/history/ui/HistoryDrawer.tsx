...
export const HistoryDrawer = ({ isOpen, onClose }: HistoryDrawerProps) => {
  ...
  const { setFOS, setSORS, setSOCH, setActiveRecord } =
  ...
  const handleLoad = (entry: HistoryEntry) => {
    ...
    if (loadedSors.length < 4) {
      const additionalSors = Array.from(
        { length: 4 - loadedSors.length },
        () => ({
      ...
      setSORS([...loadedSors, ...additionalSors]);
    }
    else if (loadedSors.length > 4) {
      setSORS(loadedSors.slice(0, 4));
    }
    else {
      setSORS(loadedSors);
    }
          ...
          {entries.length === 0 ? (
            ...
            <div className={styles.list}>
              {sortedEntries.map((entry) => (
                <div
                  key={entry.id}
                  className={`${styles.item} ${entry.isPinned ? styles.pinned : ""}`}
